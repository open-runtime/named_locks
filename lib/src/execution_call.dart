...
class ExecutionCallErrors<R, E extends Exception> {
  ...
  ({bool isSet, E? get}) get anticipated =>
      LatePropertyAssigned<E>(() => _anticipated) ? (isSet: true, get: _anticipated) : (isSet: false, get: null);

  late final Object? _unknown;

  ({bool isSet, Object? get}) get unknown =>
      LatePropertyAssigned<Object>(() => _unknown) ? (isSet: true, get: _unknown) : (isSet: false, get: null);

  late final StackTrace? _trace;

  ({bool isSet, StackTrace? get}) get trace =>
      LatePropertyAssigned<StackTrace?>(() => _trace) ? (isSet: true, get: _trace) : (isSet: false, get: null);
  ...
  ExecutionCallErrors({
    ...
    required Completer<R> this.completer,
  ...
  FutureOr<E> rethrow_() {
    if (completer.isCompleted)
      !caught || trace.isSet
          ? (throw Error.throwWithStackTrace((anticipated.get ?? unknown.get)!, trace.get!))
...
class ExecutionCall<R, E extends Exception> {
  ...
  R get returned => LatePropertyAssigned<R>(() => _returned)
      ? _returned
      : (throw Exception(
          '[returned] value is not available. To ensure property availabilities [await completer.future]. ',
        ));

  late final ExecutionCallErrors<R, E> _error;

  ({bool isSet, ExecutionCallErrors<R, E>? get}) get error =>
      LatePropertyAssigned(() => _error) ? (isSet: true, get: _error) : (isSet: false, get: null);

  late final bool _successful;

  ({bool isSet, bool? get}) get successful =>
      LatePropertyAssigned<bool>(() => _successful) ? (isSet: true, get: _successful) : (isSet: false, get: null);

  late final bool _guarded;

  ({bool isSet, bool? get}) get guarded =>
      LatePropertyAssigned<bool>(() => _guarded) ? (isSet: true, get: _guarded) : (isSet: false, get: null);
  ...
  ExecutionCall({required ExecutionCallType<R, E> callable, bool this.safe = false, this.verbose = false})
    : _callable = callable;
  ...
  ExecutionCall<R, E> execute() {
    ...
    completer.future.catchError(
      (e, trace) => LatePropertyAssigned<ExecutionCallErrors<R, E>>(() => _error)
          ? e
          : _error = ExecutionCallErrors<R, E>(
              anticipated: e is E ? e : null,
              unknown: e is! E && e is Object ? e : null,
              trace: trace,
              completer: completer,
            ),
    );
    ...
    try {
      ...
      _returned is Future<R>
          ? _returned
                ...
                .whenComplete(
                  () => _successful =
                      completer.isCompleted && !LatePropertyAssigned<ExecutionCallErrors<R, E>>(() => _error) ||
                      !(completer..completeError((_error.anticipated.get ?? _error.unknown.get)!, _error.trace.get))
                          .isCompleted,
                )
      ...
      if (verbose && _returned is Future<R>)
        print(
          'Guarded ExecutionCall has returned an asynchronous result and will complete when property completer.future is resolved.',
        );
      else if (verbose)
        print(
          'Guarded ExecutionCall returned a synchronous result and was successful: $_successful with return value: $returned',
        );
    } on E catch (e, trace) {
      ...
      _successful =
          !(completer..completeError((
                ((_error = ExecutionCallErrors<R, E>(
                      /* setting anticipated here */ anticipated: e,
                      trace: trace,
                      completer: completer,
                    )).anticipated.get ??
                    _error.unknown.get)!,
      ...
      _successful =
          !(completer..completeError((
                ((_error = ExecutionCallErrors<R, E>(
                      /*setting unknown here */ unknown: e,
                      trace: trace,
                      completer: completer,
                    )).anticipated.get ??
                    _error.unknown.get)!,
